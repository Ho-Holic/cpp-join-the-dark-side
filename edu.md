...
- заменить все знаки одного значения на другое
- найти знак который встречается чаще всего

```cpp
  int digits_in(int n);
  int find_digit(int n, int digit);
  int replace_in_number(int n, int from, int to);
  int find_max_in_number(int n, int digit)
  
```

## Использование полученных функций

- написать функцию которая в списке чисел заменяет знаки 
  { 22, 1, 145, 910, 23438} -> replace_in_sequence(1, 0) -> { 22, 0, 45, 900, 22438 } 
  
- написать функцию которая находит число в котором чаще всего встречается знак
  { 32, 8876, 23, 99888, 12} -> find_max_in_sequence(8) -> 99888
